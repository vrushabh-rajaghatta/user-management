...
public sealed partial class SigningKeyRing
{
    ...
    /// </summary>
    public const string CurrentKeySetting = "SKSMCORP_SIGNING_KEY_CURRENT";

    /// <summary>
    /// Everything after the prefix is the key identifier, so
    /// SKSMCORP_SIGNING_KEY_V1 configures the key named "v1". The accepted set
    /// is simply whatever is configured; there is no required second key, and
    /// nothing here presumes a rotation has happened.
    /// </summary>
    public const string KeyPrefix = "SKSMCORP_SIGNING_KEY_";
    ...
    /// so SKSMCORP_SIGNING_KEY_V1 and a carrier reading "v1" agree while the

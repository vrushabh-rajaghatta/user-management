using SKSMCorp.Platform.Persistence.Configurations;
...
///
/// SKSMCorpDbContextFactory is the sanctioned exception — it exists for
/// design-time migrations, which never call SaveChanges.
/// </summary>
public sealed class SKSMCorpDbContext : DbContext
{
    public SKSMCorpDbContext(DbContextOptions<SKSMCorpDbContext> options)
        : base(options)
    {
    }

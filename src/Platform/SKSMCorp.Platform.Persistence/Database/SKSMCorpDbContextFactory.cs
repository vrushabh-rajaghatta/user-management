...
///
/// It reads SKSMCORP_CONNECTION and has NO fallback. The previous version
...
/// decide that.
/// </summary>
public sealed class SKSMCorpDbContextFactory
    : IDesignTimeDbContextFactory<SKSMCorpDbContext>
{
    internal const string ConnectionVariable = "SKSMCORP_CONNECTION";

    public SKSMCorpDbContext CreateDbContext(string[] args)
    {
        ...
        if (string.IsNullOrWhiteSpace(connectionString))
        {
            throw new InvalidOperationException(
                ...
                + "Database=sksmcorp;Username=migration_role;Password=...\""
                + Environment.NewLine
                + Environment.NewLine
        ...
        var optionsBuilder = new DbContextOptionsBuilder<SKSMCorpDbContext>();

        optionsBuilder.UseNpgsql(connectionString);

        return new SKSMCorpDbContext(optionsBuilder.Options);
    }
}

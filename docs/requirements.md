...
| **CE3** | **`POST /api/users/{userId}/email`** with `{ "email": "…", "reason": "…" }` → `204`, under **`user.update`**, human callers only. `email` is required. `reason` is optional, as the catalogue says: a missing, empty or whitespace-only reason means none. Any other reason is recorded as the audit record's Reason, exactly as sent. |

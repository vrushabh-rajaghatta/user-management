...
const CARRIER_COOKIE = "__Host-sksmcorp=v1.c2Vzc2lvbg.c2lnbmF0dXJl; Path=/; Secure; HttpOnly; SameSite=Strict";
...
function temporaryDirectory(directories: string[]): string {
  const directory = mkdtempSync(path.join(tmpdir(), "sksmcorp-web-certs-"));
...
async function startProxy(cleanups: (() => Promise<void>)[], proxy: Record<string, ProxyOptions>) {
  const server: ViteDevServer = await createViteServer({
    configFile: false,
    root: mkdtempSync(path.join(tmpdir(), "sksmcorp-web-proxy-")),
    logLevel: "silent",

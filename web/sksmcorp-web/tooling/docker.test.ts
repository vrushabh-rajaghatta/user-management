 ...
 */
const PROJECTS = [
  "src/Host/SKSMCorp.Host",
  "src/Platform/SKSMCorp.Platform.Application",
  "src/Platform/SKSMCorp.Platform.Domain",
  "src/Platform/SKSMCorp.Platform.Persistence",
  "src/Shared/SKSMCorp.SharedKernel",
];
...
describe("the base Compose file", () => {
  ...
  it("trusts no proxy, leaving that to each deployment", () => {
    expect(read(BASE).services?.host?.environment?.SKSMCORP_TRUSTED_PROXIES).toBeUndefined();
  });
});
...
describe("catalogue synchronisation in the deployment chain", () => {
  ...
  it("connects as the migration role, and as nothing else", () => {
    ...
    const connection = sync?.environment?.SKSMCORP_CONNECTION ?? "";
  ...
  it("publishes the tool into the output its target copies from", () => {
    ...
    expect(dockerfile).toContain('ENTRYPOINT ["dotnet", "SKSMCorp.CatalogueSync.dll"]');
  });

  it("restores the tool's project, so the publish is not resolving it by accident", () => {
    expect(dockerfile).toContain("dotnet restore src/Tools/SKSMCorp.CatalogueSync/SKSMCorp.CatalogueSync.csproj");
  });
});
...
describe("the development overlay", () => {
  ...
  it("points the proxy at the API over the Compose network", () => {
    expect(web?.environment?.SKSMCORP_WEB_API_ORIGIN).toBe("http://host:8080");
  });

  it("tells the dev server it is in a container, so it binds every interface", () => {
    expect(web?.environment?.SKSMCORP_WEB_IN_CONTAINER).toBe("true");
  });
  ...
  it("trusts exactly the web container as a proxy", () => {
    const trusted = host?.environment?.SKSMCORP_TRUSTED_PROXIES;
  ...
  it("writes development mail to .secrets/mail through the working-tree mount, and only in the overlay", () => {
    expect(host?.environment?.SKSMCORP_MAIL_DEV_SINK_DIRECTORY).toBe("/source/.secrets/mail");
    expect(host?.environment?.SKSMCORP_PUBLIC_BASE_URL).toBe("https://localhost:5173");
    expect(host?.volumes ?? []).toContain("./:/source");
    expect(read(BASE).services?.host?.environment?.SKSMCORP_MAIL_DEV_SINK_DIRECTORY).toBeUndefined();
  });
...
describe("the developer environment's prerequisite check", () => {
  ...
  it("refuses when the certificate is absent, and says how to create one", () => {
    const result = spawnSync("./up.sh", ["--check"], {
      cwd: REPO_ROOT,
      env: { ...process.env, SKSMCORP_CERTS_DIR: "/nonexistent/certs" },
      encoding: "utf8",
    });
  ...
  it("does not create the certificate it asked for", () => {
    ...
    spawnSync("./up.sh", ["--check"], {
      cwd: REPO_ROOT,
      env: { ...process.env, SKSMCORP_CERTS_DIR: directory },
      encoding: "utf8",
    });
...
describe("the bootstrap script's choice of Compose files", () => {
  ...
  function bootstrap(containers: string[], labels: string[]) {
    const directory = mkdtempSync(path.join(tmpdir(), "sksmcorp-web-bootstrap-"));

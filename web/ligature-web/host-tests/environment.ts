...
/**
 * A whole Ligature installation, created for one run of this suite and removed
...
const ROLE_PASSWORD = "ligature-test-role";
...
/** The environment a .NET tool runs with: no inherited Ligature settings. */
function toolEnvironment(overrides: Record<string, string>): NodeJS.ProcessEnv {
  const inherited = Object.fromEntries(
    Object.entries(process.env).filter(([name]) => !name.startsWith("LIGATURE_")),
  );
...
export async function createEnvironment(): Promise<HostEnvironment> {
  databaseName = `ligature_web_${randomUUID().replace(/-/g, "")}`;
  workspace = await mkdtemp(path.join(tmpdir(), "ligature-web-host-"));
  ...
  run("dotnet", ["ef", "database", "update", "--project", "src/Platform/Ligature.Platform.Persistence"], {
    LIGATURE_CONNECTION: connectionFor("migration_role", databaseName),
  });
  ...
  // it.
  run("dotnet", ["run", "--project", "src/Tools/Ligature.AuditSchema"], {
    LIGATURE_PRIVILEGED_CONNECTION: `Host=localhost;Port=5432;Database=${databaseName};Username=postgres;Password=postgres`,
  });
  ...
  run(
    "dotnet",
    [
      "run", "--project", "src/Tools/Ligature.Provisioning", "--",
      "--first-name", "Ada",
      "--last-name", "Lovelace",
    ...
    { LIGATURE_CONNECTION: connectionFor("provisioning_role", databaseName) },
  );
  ...
  host = spawn("dotnet", ["run", "--project", "src/Host/Ligature.Host"], {
    cwd: REPO_ROOT,
    env: toolEnvironment({
      LIGATURE_CONNECTION: connectionFor("app_role", databaseName),
      LIGATURE_SIGNING_KEY_CURRENT: "v1",
      LIGATURE_SIGNING_KEY_V1: randomBytes(32).toString("base64"),
  ...
  web = spawn("npx", ["vite"], {
    cwd: APP_ROOT,
    env: toolEnvironment({ LIGATURE_WEB_API_ORIGIN: HOST_ORIGIN }),
    stdio: "pipe",
  });

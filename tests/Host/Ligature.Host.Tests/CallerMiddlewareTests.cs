...
public sealed class CallerMiddlewareTests
{
    ...
    /// </summary>
    private const string CookieName = "__Host-ligature";

    private static readonly AccessCarrier Verifier =
        Carrier("v1", ("LIGATURE_SIGNING_KEY_V1", PrimaryKey));
    ...
    private static readonly string UnknownKeyA =
        Carrier("v3", ("LIGATURE_SIGNING_KEY_V3", ForeignKey)).Issue(SessionA);
    ...
    {
        var established = await EstablishedBy(
            request => request.Headers.Cookie = $"__host-ligature={CarrierA}");
    ...
    public async Task A_case_variant_beside_the_real_cookie_is_ignored()
    {
        var established = await EstablishedBy(
            request => request.Headers.Cookie =
                $"{CookieName}={CarrierB}; __host-ligature={CarrierA}");
    ...
    {
        var established = await EstablishedBy(
            request => request.Headers.Cookie = $"ligature={CarrierA}");

...
public sealed class AuthenticationEndToEndTests
{
    ...
    /// </summary>
    private const string CarrierCookieName = "__Host-sksmcorp";
    ...
    public async Task A_case_variant_cookie_name_establishes_nothing_over_http()
    {
        await RunAsync(async (client, actor) =>
        {
            ...
            await ProbeAsync(
                client, authorization: null, cookie: $"__host-sksmcorp={sessions.CarrierB}");
            ...
            SigningKeyRing.Load(
                new ConfigurationBuilder()
                    ...
                    [
                        ...
                        new KeyValuePair<string, string?>(
                            "SKSMCORP_SIGNING_KEY_V1", HostFactory.PrimaryKey),
                    ])
                    .Build()));
    ...
    /// __Host-sksmcorp cookie — the only cookie on the response. The value is

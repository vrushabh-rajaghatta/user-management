...
public sealed class ReleaseBuildTests : IClassFixture<ReleaseBuildTests.ReleaseHost>
{
    ...
    {
        var types = TypeNames(Path.Combine(_release.Output, "SKSMCorp.Platform.Persistence.dll"));
    ...
    {
        var sink = Directory.CreateTempSubdirectory("sksmcorp-release-sink-").FullName;
    ...
    {
        var start = new ProcessStartInfo("dotnet", Path.Combine(_release.Output, "SKSMCorp.Host.dll"))
        {
        ...
        start.Environment["SKSMCORP_SIGNING_KEY_CURRENT"] = HostFactory.PrimaryKeyId;
        start.Environment["SKSMCORP_SIGNING_KEY_V1"] = HostFactory.PrimaryKey;
    ...
    {
        public string Output { get; } = Directory.CreateTempSubdirectory("sksmcorp-release-host-").FullName;

        public async Task InitializeAsync()
        {
            var project = Path.Combine(RepositoryRoot(), "src", "Host", "SKSMCorp.Host", "SKSMCorp.Host.csproj");
        ...
        private static string RepositoryRoot()
        {
            ...
            while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "SKSMCorp.slnx")))

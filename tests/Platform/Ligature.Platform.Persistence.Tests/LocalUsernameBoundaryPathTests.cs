...
/// that half is in Ligature.Provisioning.Tests (UW-10). The CHECK backing all
    ...
    private ServiceProvider Provider(List<string> lookups)
    {
        ...
        services.AddScoped<IUserIdentityRepository>(
            provider => new RecordingIdentityRepository(
                new UserIdentityRepository(provider.GetRequiredService<LigatureDbContext>()), lookups));

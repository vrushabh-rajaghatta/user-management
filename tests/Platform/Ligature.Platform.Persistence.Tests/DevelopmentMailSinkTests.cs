...
public sealed class DevelopmentMailSinkTests : IDisposable
{
    ...
    private readonly string _directory = Directory.CreateTempSubdirectory("ligature-mail-sink-").FullName;
    ...
    public async Task A_message_is_written_to_one_file_with_its_recipient_subject_and_link()
    {
        ...
        Assert.Contains("Subject: Activate your Ligature account", content, StringComparison.Ordinal);
    ...
    private static RenderedMessage Message() =>
        new("ada@example.test", "Activate your Ligature account", $"Hello Ada,\n\nActivate your account: {Link}\n");

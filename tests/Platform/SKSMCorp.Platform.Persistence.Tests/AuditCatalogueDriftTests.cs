...
/// <summary>
/// The event catalogue is reconciled by SKSMCorp.AuditSchema on every
...
/// event catalogue by SKSMCorp.AuditSchema as audit_owner, the permission
/// catalogue by SKSMCorp.CatalogueSync as migration_role.
///
...
///
/// Target database comes from SKSMCORP_CONNECTION. These tests FAIL rather
...
public sealed class AuditCatalogueDriftTests
{
    ...
    private static async Task RequireDeployedAndProvisionedAsync(NpgsqlConnection connection)
    {
        ...
        Assert.True(
            ...
            + "SKSMCorp.AuditSchema after the migrations; drift cannot be assessed.");

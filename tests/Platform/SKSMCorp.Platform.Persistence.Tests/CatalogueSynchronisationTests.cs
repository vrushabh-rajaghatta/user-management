...
public sealed class CatalogueSynchronisationTests
{
        ...
        Assert.Equal(1, await CountAsync(database, $"""
            SELECT count(*)
             ...
             WHERE r.code = 'security-administrator'
               ...
               AND rp.granted_by = '{SKSMCorp.Platform.Domain.Users.User.SystemUserId.Value}'
            """));
    ...
    public async Task A_grant_the_catalogue_does_not_list_is_refused_not_revoked()
    {
        ...
        await ExecuteAsync(database, $"""
            ...
            SELECT gen_random_uuid(), r.id, p.id, now(), '{SKSMCorp.Platform.Domain.Users.User.SystemUserId.Value}'
    ...
    private static readonly string SystemActor =
        SKSMCorp.Platform.Domain.Users.User.SystemUserId.Value.ToString();
    ...
    private static async Task<AuditBoundaryDatabase> ProvisionedAsync()
    {
        ...
        // The deployment phase, as SKSMCorp.AuditSchema performs it: the event
    ...
    /// </summary>
    private static SKSMCorpDbContext ContextFor(AuditBoundaryDatabase database, string role)

...
public sealed class UserProfileIntegrationTests : IClassFixture<ActivationDatabase>
{
    ...
    private async Task<IReadOnlyList<SKSMCorp.Platform.Application.Users.Queries.UserList.UserListRow>> ListRowsAsync()
    {
        ...
        bool inactive = false, bool withCredential = false, bool noEmail = false, bool externalOnly = false)
    {
        ...
        if (withCredential)
        {
            var hashed = new SKSMCorp.Platform.Persistence.Services.PasswordHasher().Hash("a-password-for-the-detail-test");

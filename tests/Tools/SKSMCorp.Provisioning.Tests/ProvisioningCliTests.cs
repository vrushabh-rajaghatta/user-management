namespace SKSMCorp.Provisioning.Tests;
...
public sealed class ProvisioningCliTests : IDisposable
{
    private readonly string? _originalConnection =
        Environment.GetEnvironmentVariable("SKSMCORP_CONNECTION");

    private readonly string _workspace =
        Directory.CreateTempSubdirectory("sksmcorp-provisioning-tests").FullName;

    public void Dispose()
    {
        Environment.SetEnvironmentVariable(
            "SKSMCORP_CONNECTION", _originalConnection);
    ...
    public async Task A_database_without_the_audit_schema_is_refused_with_the_remedy()
    {
        ...
        Assert.Contains("SKSMCorp.AuditSchema", run.Error);
    ...
    public async Task Help_is_not_an_error()
    {
        ...
        Assert.Equal(Program.Success, run.ExitCode);
        Assert.Contains("SKSMCORP_CONNECTION", run.Output);
    }
    ...
    {
        Environment.SetEnvironmentVariable(
            "SKSMCORP_CONNECTION", database.ConnectionString);

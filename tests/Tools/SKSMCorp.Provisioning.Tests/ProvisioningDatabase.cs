...
internal sealed class ProvisioningDatabase : IAsyncDisposable
{
    ...
    /// SKSMCORP_CONNECTION to point at a throwaway database. These tests must
    ...
    {
        var databaseName = $"sksmcorp_prov_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            if (auditDeployed)
            {
                ...
                // Deployment phase, as SKSMCorp.AuditSchema performs it: the
